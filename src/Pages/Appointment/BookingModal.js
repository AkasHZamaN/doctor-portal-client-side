...
const BookingModal = ({date, treatment, setTreatment, refetch}) => {
    ...
    const handleBooking = event =>{
        ...
        fetch('https://powerful-hollows-63277.herokuapp.com/booking', {
          method: 'POST',
          headers: {
        ...
        .then(data => {
          // console.log(data);
          if(data.success){
            toast(`Your Appointment is set at ${formattedDate} at ${slot}`)
          }
          else{
            toast.error(`Already have an Appointment is on ${data.booking?.slot}`)
          }
          //close the modal
          refetch();
        setTreatment(null);
